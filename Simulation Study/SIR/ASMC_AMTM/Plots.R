setwd("C://SFU//Research//Comparison//AdaptiveASMC//Results_Sim//SIR//ASMC_AMTM")
...
resampled_particles1 <- read.csv("ASMC_AMTM_SIR_1000_0.99_rsp.csv")
...
resampled_particles1 <- resampled_particles1 %>%
  dplyr::rename(Beta = V1, Gamma = V2, Phi = V3, S0 = V4, I0 = V5)
...
ggsave("ASMC_AMTM_SIR_DEN.jpeg", plot = p1, width = 12, height = 8, dpi = 300)

# plot for ASMC_MTM_SEIR

par(mfrow = c(2,1))
ess7 <- read.csv("ASMC_AMTM_SIR_1000_0.99_ESS.csv")
alpha7 <- read.csv("ASMC_AMTM_SIR_1000_0.99_alpha.csv")

ess7 <- ess7 %>%
  mutate(Iteration = 1:62)

ess7 <- ess7 %>%
  dplyr::select(Iteration, everything())

alpha7 <- alpha7 %>%
  mutate(Iteration = 1:63)
...
# Save the plot with high resolution
ggsave("ASMC_AMTM_SIR_ESS_Alpha.jpeg", plot = cplot1, width = 12, height = 8, dpi = 300)