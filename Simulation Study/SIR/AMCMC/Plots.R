setwd("C://SFU//Research//Comparison//AdaptiveASMC//Results_Sim//SIR//AMCMC")
...
# Read the CSV files
chain1 <- read.csv("AMCMC_SIR_chain1.csv")
chain2 <- read.csv("AMCMC_SIR_chain2.csv")
chain3 <- read.csv("AMCMC_SIR_chain3.csv")
chain4 <- read.csv("AMCMC_SIR_chain4.csv")
chain5 <- read.csv("AMCMC_SIR_chain5.csv")
...
chain1 <- chain11%>% mutate(Iteration = 1:37000)
chain2 <- chain22%>% mutate(Iteration = 1:37000)
chain3 <- chain33%>% mutate(Iteration = 1:37000)
chain4 <- chain44%>% mutate(Iteration = 1:37000)
chain5 <- chain55%>% mutate(Iteration = 1:37000)
...
parameter_names <- c("beta", "gamma", "phi", "S0", "I0")
names(combined_chains)[1:5] <- parameter_names
...
ggsave("trace_plots_SIR.jpg", plot = p, width = 12, height = 8, dpi = 300)
